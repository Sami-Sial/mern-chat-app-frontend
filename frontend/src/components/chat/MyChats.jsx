...
const MyChats = () => {
  ...
  const filterChats = (keyword, currentFilter = activeFilter) => {
    let results = chats;
    
    // Apply type filter
    if (currentFilter === "unread") {
      results = results.filter((c) => getNotif(c.users) > 0);
    } else if (currentFilter === "groups") {
      results = results.filter((c) => c.isGroupChat);
    }
  ...
  useEffect(() => {
    filterChats(search, activeFilter);
  }, [chats, notification]);
  ...
  return (
    <>
          ...
          {["all", "unread", "groups"].map((f) => (
            <button
              ...
              style={{
                ...
                borderRadius: "20px",
                border: "none",
                backgroundColor: activeFilter === f ? "#25D366" : "#202c33",
            ...
            filteredChats.map((chat) => (
              ...
              >
                {(!chat.isGroupChat ? getSenderPic(chat.users) : chat.groupPic) ? (
                  <img
                    src={!chat.isGroupChat ? getSenderPic(chat.users) : chat.groupPic}
                    style={{
                      borderRadius: "50%",
                      marginRight: "5px",
                      cursor: "pointer",
                  ...
                  <Avatar
                    sx={{
                      bgcolor: stringToColor(!chat.isGroupChat ? getSenderName(chat.users) : chat.chatName),
                      marginRight: "5px",
                      width: 40,
                      height: 40,
                      border: "1px solid #202c33",
                  ...
                  >
                    {(!chat.isGroupChat ? getSenderName(chat.users) : chat.chatName)?.charAt(0).toUpperCase()}
                  </Avatar>
                )}
      ...
      <Dialog 
        ...
        maxWidth="sm"
        PaperProps={{
          style: { borderRadius: "12px", minHeight: "60vh" }
        }}
      >
